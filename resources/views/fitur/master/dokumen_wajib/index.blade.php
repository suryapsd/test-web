...
@section('content')
  ...
  <x-modal.modal-form id="{{ $table_id }}" title="{{ $title }}" form-action="javascript:void(0)">
    ...
    <x-forms.input-field label="Dokumen Wajib" name="name" />
    <x-forms.select2-default name="type" label="Type" :datas="$types" required="true" class="" columnSpan="col-12" />
...
@push('script')
  <script>
    ...
    $('body').on('click', '.editData{{ $table_id }}', function() {
      ...
      modal.find('#type').val(data.type).trigger('change');
